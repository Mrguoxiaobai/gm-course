...
var app = new Vue({
            ...
            $.ajax({
                ...
                success : function(data) {
                    if (data.code === 200) {
                        ...
                        level += "</select></div>";
                        $("#pxm").append(level);
                        $("#pxm option[value='"+pxmName+"']").attr("selected","selected");
                    }
                }
            });
        },
    ...
    mounted:function () {
        if ($("#psex").val() !== "") {
            $("[name='psex'][value="+$("#psex").val()+"]").prop("checked", "checked");
        }
        if ($("#pinform").val() !== "") {
            $("[name='pinform'][value="+$("#pinform").val()+"]").prop("checked", "checked");
        }
        this.getXmName($("#pxm").val());
        this.validateRule();
...
function updatePositive() {
    var positive = {
        ...
        'pcode': $("#pcode").val(),
        'pname': $("#pname").val(),
        'psex': $("#psexs input:radio:checked").val()===undefined?"":$("#psexs input:radio:checked").val(),
        'pinform': $("#pinforms input:radio:checked").val()===undefined?"":$("#pinforms input:radio:checked").val(),
        'page': $("#page").val(),
    ...
    if (isMobileEmail($("#pphone").val())){
        $.ajax({
            cache: true,
            type: "POST",
            url: context + 'positive/updatePositive',
